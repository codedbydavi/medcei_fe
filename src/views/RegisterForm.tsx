...
const RegisterForm = ({ onSwitchToLogin }: RegisterFormProps) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!passwordRegex.test(password)) {
      toast.error(
        "A senha deve conter no mínimo 8 caracteres, incluindo números e caracteres especiais.",
      );
      return;
    }
    ...
    try {
      ...
      if (error.code === "auth/weak-password")
        message = "A password é muito fraca.";
      ...
      <div className="bg-white p-4 sm:p-6 md:p-10 rounded-xl sm:rounded-2xl md:rounded-[2rem] shadow-2xl w-full max-w-[500px] flex-shrink-0">
        ...
        <form className="space-y-3" onSubmit={handleRegister}>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-0.5">
              ...
              <div className="relative">
                ...
                />
                {isFilled(firstName) && (
                  <CheckCircle2 className="absolute right-2 top-2 w-4 h-4 text-teal-500" />
                )}
              </div>
            </div>
            <div className="space-y-0.5">
          ...
          <div className="flex items-start gap-2 py-1">
            ...
            <label
              htmlFor="terms"
              className="text-[10px] text-slate-500 leading-tight cursor-pointer"
            >
              Aceito os <span className="text-teal-600 font-bold">Termos</span>{" "}
              e a <span className="text-teal-600 font-bold">Privacidade</span>
            </label>
          </div>
