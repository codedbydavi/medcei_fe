...
const LoginForm = ({ onSwitchToRegister }: LoginFormProps) => {
  ...
  const handleResendEmail = async () => {
    try {
      setLoading(true);
      await authService.sendVerificationAgain(email, password);
      toast.success(
    ...
    <>
      <div className="bg-white p-4 sm:p-6 md:p-10 rounded-2xl sm:rounded-[2rem] shadow-xl w-full max-w-[500px] flex-shrink-0">
        <div className="flex flex-col items-center mb-4 sm:mb-6 md:mb-8 text-center">
          ...
          <p className="text-xs text-slate-400 mt-0.5 sm:mt-1 uppercase">
        ...
        >
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-0.5 sm:mb-1 ml-1 uppercase tracking-wider">
              Email
            </label>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 text-xs px-0.5 sm:px-1">
            <label className="flex items-center space-x-2 cursor-pointer select-none">
              <input
                ...
                className="w-4 h-4 text-teal-600 rounded border-gray-300 focus:ring-teal-500 accent-teal-500"
              />
        ...
        <div className="relative my-4 sm:my-6">
          ...
          <div className="relative flex justify-center">
            <span className="px-3 sm:px-4 bg-white text-xs text-slate-400 uppercase tracking-wider">
              Ou continue com
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-2 sm:gap-3">
          <button className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 border border-slate-200 py-2 sm:py-2.5 rounded-lg sm:rounded-xl hover:bg-slate-50 transition-colors bg-white shadow-sm">
            ...
            />
            <span className="font-bold text-slate-600 text-xs hidden sm:inline">
              Google
            </span>
          </button>
          <button className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 border border-slate-200 py-2 sm:py-2.5 rounded-lg sm:rounded-xl hover:bg-slate-50 transition-colors bg-white shadow-sm">
